...
public class TexAutoIndentStrategy extends DefaultAutoIndentStrategy {
		...
		TexlipsePlugin.getDefault().getPreferenceStore().addPropertyChangeListener(new  
			...
			public void propertyChange(PropertyChangeEvent event) {
				...
				if (TexlipseProperties.WORDWRAP_LENGTH.equals(ev)) {
					ll = TexlipsePlugin.getDefault().getPreferenceStore().getInt(TexlipseProperties.WORDWRAP_LENGTH);
					setLineLength(ll);
				}
				if (TexlipseProperties.TEX_ITEM_COMLETION.equals(ev)) {
					autoItem = TexlipsePlugin.getDefault().getPreferenceStore().getBoolean(TexlipseProperties.TEX_ITEM_COMLETION);
				}
				if (TexlipseProperties.INDENTATION_LEVEL.equals(ev) || TexlipseProperties.INDENTATION.equals(ev) || TexlipseProperties.INDENTATION_ENVS.equals(ev)) {
					setIndetationPreferenceInfo(TexlipsePlugin.getDefault().getPreferenceStore());
				}
			}; 
		});
	...
	private void setIndetationPreferenceInfo(IPreferenceStore editorPreferenceStore) {
		Boolean indentation = new Boolean(TexlipsePlugin.getPreference(TexlipseProperties.INDENTATION));
		...
		this.autoItem =  TexlipsePlugin.getDefault().getPreferenceStore().getBoolean(TexlipseProperties.TEX_ITEM_COMLETION);
	}
	...
	public void customizeDocumentCommand(IDocument document, DocumentCommand command) {
		if (this.indent) {
			if (command.length == 0 && command.text != null && this.tools.endsWithNewline(document, command.text) && itemSetted && autoItem) {
				dropItem(document, command);
			
			} else if (command.length == 0 && command.text != null && this.tools.endsWithNewline(document, command.text)) {
	 ...
	 * @param length New line length value.
	 */
	...
	/**
	 * Sets new tabwidth.
	 * @param length Number of spaces in tab.
	 */
	...
	private void smartIndentAfterNewLine(IDocument document, DocumentCommand command) {
		try {
			...
			String endLine = document.get(commandOffset, lineLength + lineOffset - commandOffset);
			
			//test if line contains \begin and  search the environment (itemize, table...)
			int beginIndex;
			int endIndex;
			if ((beginIndex = startLine.indexOf("\\begin")) != -1) {
				String endText = this.tools.getEndLine(startLine.substring(beginIndex), "\\begin");
				StringBuffer buf = new StringBuffer(command.text);
				String prevIndentation = "";
				
				if (Arrays.binarySearch(this.indentationItems, this.tools.getEnvironment(endText)) >= 0) {
					prevIndentation = this.tools.getIndentation(document, line, "\\begin", this.tabWidth);
					buf.append(prevIndentation);
					buf.append(this.indentationString);
					
				}
				if ((this.tools.getEnvironment(endText).equals("itemize") ||
						this.tools.getEnvironment(endText).equals("enumerate"))&& autoItem) {
					buf.append("\\item ");
					itemSetted = true;
					itemAtLine = document.getLineOfOffset(command.offset);
				} else if (this.tools.getEnvironment(endText).equals("description") && autoItem) {
					command.shiftsCaret = false;
					command.caretOffset = command.offset+buf.length()+6; // 6 = "\item[".length
					buf.append("\\item[]");
					itemSetted = true;
					itemAtLine = document.getLineOfOffset(command.offset);
				}
				command.text = buf.toString();
				//test if line contains \end and  search the environment (itemize, table...)
			} else if ((endIndex = startLine.indexOf("\\end")) != -1) {
				String endText = this.tools.getEndLine(startLine.substring(endIndex), "\\end");
				
				if (Arrays.binarySearch(this.indentationItems, this.tools.getEnvironment(endText)) >= 0) {
					int matchingBegin = this.tools.findMatchingBeginEquation(document, line, this.tools.getEnvironment(endText));					
					String prevIndentation = this.tools.getIndentation(document, matchingBegin, "\\begin", this.tabWidth);
					...
					(this.tools.getIndentation(document, command).length() - prevIndentation.length()) +
					this.tools.getLineDelimiter(document, command).length() +
					prevIndentation.length();
	 ...
	 * @param document Document where new line is detected.
	 * @param command Command that represent the change of the document (here command text is "}").
	 */
	private void smartIndentAfterBracket(IDocument document, DocumentCommand command) {
		try {
			int commandOffset = command.offset;
			int line = document.getLineOfOffset(commandOffset);
			int lineOffset = document.getLineOffset(line);
			int lineLength = document.getLineLength(line);
			String lineText = document.get(lineOffset, lineLength); //the original line text		
			String text = lineText.trim().concat(command.text); //modified linetext
			...
			if (text.startsWith("\\end")) {			
				...
				if (Arrays.binarySearch(this.indentationItems, this.tools.getEnvironment(endLine)) >= 0) {
					int matchingBegin = this.tools.findMatchingBeginEquation(document, line, this.tools.getEnvironment(endLine));
					StringBuffer buff = new StringBuffer(this.tools.getIndentation(document, matchingBegin, "\\begin", this.tabWidth));
					buff.append(text);
					command.offset = lineOffset;
	...
	/**
	 * Insets an \item or an \item[] string
	 * Works ONLY it \item is found from the beginin of the previous line
	 * @param d	
	 * @param c
		...
		try{
			...
			while(lines > 1){
				...
				if (prevLine.trim().startsWith("\\item[")) {
					buf.append("\\item[]");
					c.shiftsCaret = false;
					c.caretOffset = c.offset +tools.getIndentation(d,c).length()+ 6 + c.text.length(); 
					c.text = buf.toString();
					itemSetted = true;
